class Formatter
  ...
  fixPreformattedText: ($content) ->
    ...
    $content
      ...
      .end()
      .find('div.message-content p').each (i, el) =>
          $(el)[0].name = "blockquote"
      .end()
      .find('strong').each (i, el) =>
    ...
    $content
      .find('a').each (i, el) =>
        ...
        if $(el).hasClass 'added-anchor'
          ...
          $(el).attr 'href', "#" + $(el).text().toLowerCase().replace(/\?/, '').replace(/'/, '').split(' ').join('-')
      .end()
